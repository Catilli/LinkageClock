...
/**
 * AJAX handler for getting employee profile data
 */
function linkage_ajax_get_employee_profile() {
    if (!wp_verify_nonce($_POST['nonce'], 'linkage_dashboard_nonce')) {
    ...
    // Get employee status
    $employee_status = linkage_get_employee_status_from_database($employee_id);
    
    // Get employee meta data
    $position = get_user_meta($employee_id, 'linkage_position', true);
    $company_id = get_user_meta($employee_id, 'linkage_company_id', true);
    
    // Get role display
    $role_display = linkage_get_user_role_display($employee_id);
    
    // Format last action time
    $last_action = 'Never';
    if ($employee_status->last_action_time && $employee_status->last_action_time !== 'Never') {
        $last_action = linkage_format_actual_time($employee_status->last_action_time);
    }
    ...
    $recent_attendance = $wpdb->get_results($wpdb->prepare("
        SELECT work_date, time_in, time_out, total_hours, status
    ...
    foreach ($recent_attendance as $record) {
        $formatted_attendance[] = array(
            'work_date' => date('M j, Y', strtotime($record->work_date)),
            'time_in' => $record->time_in ? date('g:i A', strtotime($record->time_in)) : null,
            'time_out' => $record->time_out ? date('g:i A', strtotime($record->time_out)) : null,
            'total_hours' => $record->total_hours ? number_format(floatval($record->total_hours), 2) : '0.00',
            'status' => $record->status
        );
    }
    
    $profile_data = array(
        'name' => linkage_get_user_display_name($employee_id),
        'email' => $employee->user_email,
        'position' => $position ?: 'Not set',
        'role' => $role_display,
        'hire_date' => date('M j, Y', strtotime($employee->user_registered)),
...
add_action('wp_ajax_linkage_get_employee_profile', 'linkage_ajax_get_employee_profile');
